...
#saved as Ariana Horn - activity3
...
environment_data <- data.frame("H" = Height, "W" = Weight)
...
ggplot(environment_data, aes(x=Height, y=Weight)) + 
  geom_point()

#Optional 'Just for fun' TASK: Change the arguments for the geom point and see what happens to the plot
ggplot(environment_data, aes(x=Height, y=Weight)) + 
  geom_point(shape = 18, colour = "green", fill = "lightblue", size = 5, stroke = 10)
